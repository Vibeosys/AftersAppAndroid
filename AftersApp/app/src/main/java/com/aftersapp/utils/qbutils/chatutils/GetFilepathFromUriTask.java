package com.aftersapp.utils.qbutils.chatutils;
...
import com.aftersapp.interfaces.chatinterfaces.SchemeType;
import com.aftersapp.utils.qbutils.ImageUtils;
...
public class GetFilepathFromUriTask extends BaseAsyncTask<Intent, Void, File> {
    ...
    public File performInBackground(Intent... params) throws Exception {
        ...
        if (SchemeType.SCHEME_CONTENT.equalsIgnoreCase(uriScheme) && !isFromGoogleApp && !isKitKatAndUpper) {
            ...
            Cursor cursor = AftersAppApplication.getInstance().getContentResolver().query(uri, filePathColumn, null, null, null);
            if (cursor != null) {
